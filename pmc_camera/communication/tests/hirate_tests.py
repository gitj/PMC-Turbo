...
class HirateTests(unittest.TestCase):
    ...
    def good_buffer_test(self):
        encoded_data = cobs_encoding.encode_data(self.random_msg)
        chunks = hirate_sending_methods.chunk_data_by_size(chunk_size=1000, file_id=0, data=encoded_data)
        buffer = ''
        for chunk in chunks:
            buffer +=chunk
        packets = hirate_receiving_methods.get_packets_from_buffer(buffer)
        files = hirate_receiving_methods.packets_to_files(packets)
        decoded_data = cobs_encoding.decode_data(files[0])
    ...
    def two_files_shuffled_together_test(self):
        encoded_data = cobs_encoding.encode_data(self.random_msg)
        chunks0 = hirate_sending_methods.chunk_data_by_size(chunk_size=1000, file_id=0, data=encoded_data)
        chunks1 = hirate_sending_methods.chunk_data_by_size(chunk_size=1000, file_id=1, data=encoded_data)
        ...
        decoded_data0 = cobs_encoding.decode_data(files[0])
        ...
        decoded_data1 = cobs_encoding.decode_data(files[1])
