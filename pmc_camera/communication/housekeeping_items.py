...
def get_collectd_items(csv_filename=None):
    ...
    if csv_filename:
        with open(csv_filename,'w') as fh:
            fh.write(','.join(['partial_glob', 'column_name', 'min_value', 'mean_value','max_value','std_value'])+'\n')
            for row in result:
...
def get_items(partial_glob,csv_filename=None):
    ...
    for colname in df.columns:
        ...
        if col.dtype == 'O': # string columns are Objects
            ...
            mean = col.mean()
            std = col.std()
        result.append((partial_glob,colname,min,mean,max,std))
    if csv_filename:
        with open(csv_filename,'w') as fh:
            fh.write(','.join(['partial_glob', 'column_name', 'min_value', 'mean_value','max_value','std_value'])+'\n')
            for row in result:
...
def get_charge_controller_items(csv_filename=None):
    return get_items(partial_glob='/home/pmc/logs/housekeeping/charge_controller/*-??_??????.csv',csv_filename=csv_filename)
