        ...
        if counter is None:
            if self.filename is None:
                ...
                raise RuntimeError("Cannot create a new counter '%s' for collection '%s' because the counter log file "
                                   ...
                                   % (item, self.name, self.counters.keys()))
        return counter
    ...
    def _write(self):
        ...
        if not self.filename:
            ...
            with open(self.filename, 'a') as fh:
                csv.writer(fh, quoting=csv.QUOTE_NONE).writerow(['epoch'] + self.counters.keys())
        with open(self.filename, 'a') as fh:
            csv.writer(fh, quoting=csv.QUOTE_NONE).writerow(['%f' % epoch] + [str(x) for x in self.counters.values()])

    def __repr__(self):
        items = [repr(value) for (key, value) in self.counters.items()]
        if not items:
            items = ['<empty>']
