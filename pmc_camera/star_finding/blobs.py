...
def process_image(im,blob_threshold=6,kernel_size=8,kernel_sigma=2,stamp_size=32, fit_blobs=True):
    #make kernel
    selm = cv2.getGaussianKernel(kernel_size,sigma=kernel_sigma)
    selm = np.outer(selm,selm)
    selm = selm/selm.sum()
    npix = selm.shape[0]*4-2
    selm[0,:] = -1.0/npix
    selm[:,0] = -1.0/npix
    selm[-1,:] = -1.0/npix
    selm[:,-1] = -1.0/npix
    ...
    #find blobs
    blobs = find_blobs(imlev.astype('float32'),threshold=blob_threshold*noise)
    #fit blobs
    if fit_blobs:
        for blob in blobs:
