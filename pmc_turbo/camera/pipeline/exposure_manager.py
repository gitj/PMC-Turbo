...
class ExposureManager(Configurable):
    ...
    max_exposure = Int(default_value=100000,min=0).tag(config=True)
    ...
    def check_exposure(self, pipeline_status, file_statistics):
        ...
        if current_exposure != file_exposure:
            logger.info("Current exposure reported by pipeline is %d, but latest file %s shows %d, so skipping exposure update"
